...
from tensorflow_use_LSTMs import MyPrediction, inputData
''' ==================== ↓  控制   ↓ ==================== '''
class Controller(QtWidgets.QMainWindow):
    ...
    def connect(self):  # 绑定信号槽
        ...
        self.ui.toolButton_normalization.clicked.connect(self.toolButton_normalization_clicked)
    ...
    def menu_opeanFile(self):
        ...
        if fileName == '' :
            ...
            if(reply != 0):
                ...
                self.out = self.prediction.Prediction(train_data[7], train_labels[7])
                self.paintLSTM.setAx(0, 5, '2d')
                self.paintLSTM.paint(0, self.out)

                self.out = self.prediction.Prediction(train_data[0], train_labels[0])
                self.paintLSTM.setAx(1, 5, '2d')
                self.paintLSTM.paint(1, self.out)
                
                self.out = self.prediction.Prediction(train_data[1], train_labels[1])
                self.paintLSTM.setAx(2, 5, '2d')
                self.paintLSTM.paint(2, self.out)

                self.out = self.prediction.Prediction(train_data[2], train_labels[2])
                self.paintLSTM.setAx(3, 5, '2d')
                self.paintLSTM.paint(3, self.out)

                self.out = self.prediction.Prediction(train_data[3], train_labels[3])
